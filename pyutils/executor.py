...
@deprecated(
    version="0.2.7",
    reason="Please use 'from pyutils.fsext import detect_encoding' instead.",
)
...
class Executor:
    ...
    def common_error_out(self, result, exit_code=-1):
        ...
        """
        error_message = result.error if result.error.strip() != "" else result.out_str
    ...
    def execute_with_tempfile(
        ...
        tempfile_ext,
        ignore_error=False,
        use_direct_stdout=False,
        custom_communicate=False,
        env=None,
        shell=True,
        work_dir: str = None,
        ...
        tf = tempfile.mkstemp(
            suffix=tempfile_ext, prefix=None, dir=self.path_to_temp_dir(), text=True
        )
        ...
        result = self.execute_file(
            ...
            use_direct_stdout=use_direct_stdout,
            custom_communicate=custom_communicate,
            env=env,
            shell=shell,
            wrap_blank_with_double_quotes=wrap_blank_with_double_quotes,
        )
        os.close(tf[0])
        os.unlink(tf[1])
        self.__restore_cwd()
    ...
    def execute_by_git_bash(
        ...
        Args:
            ...
            shell (bool, optional): Popen shell argument. Defaults to True.
        """
        return self.execute_with_tempfile(
            cmd,
            args,
            ".sh",
            ignore_error=ignore_error,
    ...
    def execute_straight(
        ...
        if wrap_blank_with_double_quotes:
            ...
            cmd = (
                cmd
                if not shell
                or re.search(r"\s", cmd) is None
                or re.search(r"\"", cmd) is not None
                else f'"{cmd}"'
            )
            if isinstance(args, list):
                args = [
                    ...
                    or re.search(r"\"", arg) is not None
                    else f'"{arg}"'
                    for arg in args
                ]
        ...
        process = subprocess.Popen(
            popen_args,
            stdout=sys.stdout if use_direct_stdout else subprocess.PIPE,
        ...
        if custom_communicate:
            ...
            if self.verbose:
                logger.info(
                    f"<= Finished: {os.path.basename(cmd)} {time.time() - start_time:.2f} seconds",
                    True,
                )
    ...
    def set_env_win(self, key: str, value: str, override=False):
        ...
        if value[-1] == "\\":
            value = value[:-1] + "/"
        # 运行设置环境变量命令
        bat_cmd = self.ADD_ENV.format(
            user="me", key=key, value=value, override=override
        )
        tf = tempfile.mkstemp(
            suffix=".bat", prefix=None, dir=self.path_to_temp_dir(), text=True
        )
    ...
    def get_git_path(self):
        ...
        if git_path is None:
            logger.info(
                "=> Begin searching git path, it may take a long time...", False
            )
            # windows 先搜索默认路径
            if shd.is_win():
    ...
    def get_unity_path(self, full_version_str=None):
        ...
        unity_path_config_yaml = Path(
            os.path.join(Path.home(), "unity_path_config.yaml")
        )
        ...
        if unity_exec_location is None:
            if shd.is_win():
                search_pattern = (
                    f"/Program*/**/{full_version_str}/Editor/Unity.exe"
                    if full_version_str is not None
                    else "/Program*/**/Editor/Unity.exe"
                )
                unity_exec_location = fsext.search(search_pattern)
                if unity_exec_location is None:
                    search_pattern = (
                        f"/**/{full_version_str}/Editor/Unity.exe"
                        if full_version_str is not None
                        else "/**/Editor/Unity.exe"
                    )
                    unity_exec_location = fsext.search(search_pattern)
            else:
                search_pattern = (
                    f"/Applications/**/Editor/{full_version_str}/Unity.app/Contents/MacOS/Unity"
                    if full_version_str is not None
                    else "/Applications/**/Unity.app/Contents/MacOS/Unity"
                )
                unity_exec_location = fsext.search(search_pattern)
    ...
    @deprecated(
        version="0.2.9",
        reason="You should always use importlib and sys.modules for python module execution.",
    )
    def execute_module(self, module, *module_parameters):
        ...
        logger.info(
            "<= Finished: {0} {1:.2f} seconds ".format(
                module_name, time.time() - start_time
            ),
            True,
        )
        return result
