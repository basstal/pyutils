...
class AutoUpgrade(object):
    ...
    def __init__(self, pkg, index=None, verbose=False, custom_get_highest_version=None):
        ...
        self.get_highest_version = (
            custom_get_highest_version
            if custom_get_highest_version is not None
            else self._get_highest_version
        )

    def upgrade_if_needed(
        self, restart=True, dependencies=False, force_reinstall=False
    ):
    ...
    def _upgrade(self, target_version, dependencies=False, force_reinstall=False):
        ...
        executor.execute_straight(
            executable, pip_args, wrap_blank_with_double_quotes=True
        )
    ...
    def check_if_later_version_exist(self):
        ...
        if self.verbose:
            logger.info(
                f"highest({highest}) > current({current}) : {highest > current}"
            )
    ...
    def search_wheel_version(self, text):
        ...
        pattern = (
            rf"({self.pkg})-(?P<version>[0-9]+\.[0-9]+\.[0-9]+)(-[\w\.]+)?(-.*\.whl)"
        )
